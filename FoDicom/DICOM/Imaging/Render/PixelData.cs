...
namespace Dicom.Imaging.Render
{
    ...
    public interface IPixelData
    {
		...
		void RenderToStream(ILUT lut, System.IO.Stream stream);
    }
    ...
    public class GrayscalePixelDataU8 : IPixelData
    {
		...
		public void RenderToStream(ILUT lut, System.IO.Stream stream)
		{
			using (var writer = new System.IO.BinaryWriter(stream,System.Text.Encoding.UTF8,true))
			{
				for (int h = Height-1; h >= 0; --h)		//dcm转bmp时h应该逆序排列
				{
		...
		public void RenderToStream(ILUT lut, System.IO.Stream stream)
		{
			using (var writer = new System.IO.BinaryWriter(stream, System.Text.Encoding.UTF8, true))
			{
				for (int h = Height-1; h >= 0; --h)		//dcm转bmp时h应该逆序排列
				{
					...
					{
						int pixel = lut == null ?
							(Data[p++] << 16) | (Data[p++] << 8) | Data[p++]
							:(lut[Data[p++]] << 16) | (lut[Data[p++]] << 8) | lut[Data[p++]];
						writer.Write(pixel);
					}
				}
			}
